...
Masthead: the supplied logo once (J and globe, **The Daily Jenya**, **YOUR MORNING EDITION** baked into the artwork). Do not add a second HTML tagline. The supplied PNG is drawn light on the navy mast with `brightness(0) invert(1)` and opacity `0.87`. Do not add `mix-blend-mode`; phones drop that blend and the logo disappears. Then the edition date, Jerusalem time before any other clock, and the language switcher. Under that, the section nav: חדשות · שווקים · AI · ארכיון. Then the indices crawl (duplicated row, 52s, pause, reduced-motion static), then one quotes line (what the news cut is, and that the markets figure is a snapshot, not a live quote).
