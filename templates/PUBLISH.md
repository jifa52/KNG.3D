...
Masthead: the supplied logo once (J and globe, **The Daily Jenya**, **YOUR MORNING EDITION** baked into the artwork). Do not add a second HTML tagline. The mast uses `assets/daily-jenya-logo-white.svg` (white lockup, transparent). Opacity `0.87` only. Do not add `filter` or `mix-blend-mode`; a brightness/invert filter turns the edge into stairs, and phones drop screen blends so the logo disappears. Then the edition date, Jerusalem time before any other clock, and the language switcher. Under that, the section nav: חדשות · שווקים · AI · ארכיון. Then the indices crawl (duplicated row, 52s, pause, reduced-motion static), then one quotes line (what the news cut is, and that the markets figure is a snapshot, not a live quote).
