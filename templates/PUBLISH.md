...
The page is night paper (`#101f2c`) on `#09141e`. Ink is `#d4dce1`. On the home sheet the mast and the indices crawl run edge to edge (gradient `#102637` to `#142e43`, gold rule `#b99a55`). The story column stays in the wrap. Nav is `#132635`. Why-it-matters sits on pale `#1b3040`. Crawl prices use `#9bc9ba` up and `#e6a5b1` down. AI sits on `#262e42`. Rules are `#344653`. News kickers are gold `#dfbd72`. No cream paper, no forest-and-lime chrome, no quote-board dashboard.
